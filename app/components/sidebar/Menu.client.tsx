...
export function Menu() {
  ...
  const loadEntries = useCallback(async () => {
    ...
    try {
      ...
      if (data.chats) {
        // convert server chat format to history item format
        const historyItems: ChatHistoryItem[] = data.chats.map((chat: ChatItem) => ({
          id: chat.id,
          urlId: chat.id,
          description: chat.title || 'Untitled Chat',
          messages: [], // empty for sidebar display
          timestamp: chat.updatedAt,
        }));

        setList(historyItems);
      }
    } catch (error) {
  ...
  const deleteItem = useCallback(
    async (event: React.UIEvent, item: ChatHistoryItem) => {
      ...
      try {
        ...
        formData.append('chatId', item.id);
        ...
        if (response.ok) {
          ...
          if (window.location.search.includes(`chatId=${item.id}`)) {
